    ...
    sphereModel()
{
    ...
    // Add shaders as observers to the light for lighting updates
    ...
    // Add shaders as observers to the camera for view and projection updates
...
ShaderShowcaseScene::~ShaderShowcaseScene() {
    // Clean up any resources specific to ShaderShowcaseScene if needed
    delete sceneLight;  // Assuming sceneLight was dynamically allocated
}


void ShaderShowcaseScene::createShaderShowcase() {
    float offset = -5.0f;  // Start closer to the camera
...
void ShaderShowcaseScene::addObjectWithShader(const Model& model, const glm::vec3& position, ShaderProgram& shader, float scale) {
    ...
    transform.setScale(glm::vec3(scale));  // Apply scale here
...
void ShaderShowcaseScene::render(const glm::mat4& projection, const glm::mat4& view, const glm::vec3& viewPos) {
    ...
    glm::vec3 uniformColor = glm::vec3(0.5f, 0.8f, 0.3f); // Set this as the same color for all objects

    glClearColor(0.2f, 0.2f, 0.2f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    // Render with the constant shader
    constantShader.use();
    constantShader.setUniform("objectColor", uniformColor);  // Set uniform color
    objects[0].draw(projection, view);

    // Render with the Lambert shader
    lambertShader.use();
    lambertShader.setLightingUniforms(lightPos, viewPos, lightColor, uniformColor);  // Apply uniform color to lighting
    ...
    // Similarly, apply the uniform color for other rows (Bushes and Spheres)
